...
export const stripePaymentController = async (req, res) => {
  try {
    ...
    // Validate cart and token
    if (!cart || !token) {
      return res.status(400).send({ error: 'Cart and token are required' });
    }
    ...
    const order = new orderModel({
      ...
      buyer: req.user._id,  // Assuming user is authenticated and `req.user` is populated
    });
    await order.save();
